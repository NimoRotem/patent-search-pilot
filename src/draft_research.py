...
def drafting_request(round_no: int, reading: Mapping[str, Any],
    ...
    if reading.get("ok"):
        ...
        if uncovered:
            lines += ["", "NO reference disclosed these elements. They are the strongest ground "
                          ...
                          "limitations. They are quoted in the SEARCH's words, not the "
                          "inventor's: treat each as a pointer to look in input/disclosure.md, "
                          "never as text to import:"]
    ...
    lines += [
        ...
        "",
        "DO NOT INVENT SUPPORT FOR THE FEATURE YOU CHOOSE. This is the way this instruction goes "
        "wrong, and it has cost whole rounds twice: asked to distinguish the art, a drafter "
        "reaches for something the disclosure gestures at and then writes what would make it "
        "work. First it was a definition, a sign convention and a sensing location for a term the "
        "inventor used once. Then it was new structure: a suction passage, a sensing branch, a "
        "sealed region, each given a numeral and a location nobody disclosed. The source-fidelity "
        "review rejects the whole turn for either, so the round buys nothing.",
        "",
        "THE HARD RULE THAT FOLLOWS: introduce NO new numbered part, passage, region, port, "
        "chamber, surface or other structure, and no new definition, sign convention, reference "
        "point or operating equivalence. Every reference numeral you rely on must ALREADY be in "
        "draft/numerals.md and trace to an affirmative passage in input/disclosure.md. What you "
        "may do is recite an element that is already disclosed more precisely, in the "
        "disclosure's own words, or claim a relationship between elements that the disclosure "
        "already states.",
        "",
        ...
        "the claims alone. A round that reports honestly that the art is close is worth more than "
        "a round that invents its way to a lower number, because the second one cannot be filed.",
        "",
