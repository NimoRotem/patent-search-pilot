...
def _run_phase(tasks, bound, wide=None, remote_bound=None, pass_key=""):
    ...
    * Cancellation is checked BEFORE a task is submitted and again on the worker thread just
    ...
    #  A restored channel costs no slot on either lane, which is the point: a resumed run must not
    #  wake a shard it already has the hits from.
    ...
    for name, fn, lane in norm:
        ...
        if banked is not None:
            ...
            pending.append((name, fn, lane))
    if restored:
        print(f"[resume] {len(restored)} retrieval channel(s) reloaded from the hit ledger "
              f"instead of re-run: {', '.join(restored)}", flush=True)

    futures = [(name, pools[lane]().submit(guarded(name, fn, gates[lane])))   # all submitted first
               for name, fn, lane in pending]
    ...
    #  Over `norm`, not `tasks`: a task may arrive as (name, fn) or (name, fn, lane), and the cold
    #  tier submits the three-element form.
...
class Retriever(DenseMixin, LexicalMixin, ExactMixin, CpcMixin, CitationMixin, QbeMixin,
    ...
    def search(self, query, subject=None, mode=None, config="hybrid",
        ...
        raw = _run_phase(p1, bound, wide=wide_profile, pass_key=pass_key)
        ...
        raw2 = _run_phase(p2, bound, wide=wide_profile, pass_key=pass_key)
