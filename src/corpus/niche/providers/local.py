...
class LocalDiscoverySource:
    ...
    def __init__(self, connection_factory, *, id_window: int | None = None, graph_limit: int | None = None):
        self.connection_factory = connection_factory
    ...
    def seed_records(self, groups, watermarks, limit):
        ...
        start = int(watermarks.get("publication_id", 0))
        connection = self._connection()
        try:
            with connection.cursor() as cursor:
        ...
        if start >= max_id:
            return [], {
                ...
                "source_max_publication_id": max_id,
            }
