...
class AgentConfig:
    ...
    #  FAST MODE. The prior-art search, as opposed to the claim attack: a handful of short
    #  whole-invention queries, no novelty clusters, no per-limitation queries, no construction
    #  queries, no refinement rounds. It keeps the fusion and the cross-encoder head, because
    #  those ARE the top-20 ranking it owes the reader.
    #
    #  Measured shape: 8 passes against ~50, one model call against ~30. See search_mode.
    fast_mode: bool = False
    #  How many whole-invention queries FAST issues besides the brief itself.
    fast_queries: int = int(os.environ.get("FAST_QUERY_SET", "4"))
    #  How many elements FAST attributes evidence to. Attribution only; it does not rank.
    fast_elements: int = int(os.environ.get("FAST_ELEMENTS", "3"))
...
class CoverageAgent:
    ...
    def _run(self, query_text, subject=None, mode="novelty", cfg: AgentConfig = None,
        ...
        if cfg.fast_mode:
            search_max = 1 + int(cfg.fast_queries) + int(cfg.fast_elements)
        else:
        ...
        #  NOT IN FAST. The structured claim reading is what the attack spends its budget
        #  from; a prior-art search never looks at a limitation, so paying a strong-tier model
        #  call to split them would be work with no consumer.
        analysis = {"units": [], "clusters": []}
        if cfg.input_claims and not cfg.fast_mode:
            try:
        ...
        try:
            specs = query_set.build(query_text, elements=elements,
                                    claims=([] if cfg.fast_mode else (cfg.input_claims or [])),
                                    spec_text=("" if cfg.fast_mode else (cfg.input_spec or "")),
                                    analysis=analysis)
        except Exception:
            traceback.print_exc()
            specs = []
        #  FAST TAKES THE HEAD OF THE PRIORITY ORDER AND STOPS. `specs` is ordered by what each
        #  query is worth, so the first few are the essence and the alternate vocabularies: the
        #  whole invention, said several ways, which is exactly what a prior-art search wants.
        if cfg.fast_mode:
            specs = [s for s in specs if s.kind in ("essence", "alt", "brief")]
        ...
        if cfg.find_mode:
            ...
            extra = extra[:5]
        if cfg.fast_mode:
            extra = extra[:max(0, int(cfg.fast_queries))]
        if extra:
        ...
        if cfg.fast_mode:
            n_element_passes = max(0, int(cfg.fast_elements))
        elif cfg.input_claims:
            try:
                import search_settings as _ss
