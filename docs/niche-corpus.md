...
Apply [sql/niche/001_fetch_queue.sql](../sql/niche/001_fetch_queue.sql) only to
...
`NICHE_SOURCE_DATABASE_URL` should use a database role that cannot write to the
active corpus. When a dedicated role is unavailable, the runtime DSN must force
`default_transaction_read_only=on`. The local provider also opens every
transaction as read only and rejects mutation statements before execution.
...
Increase only the provider intended for that run. The in-process credit ledger
reserves the worst-case request cost atomically before any network call. It is
a per-process limit, so a multi-VM deployment must keep these paid caps at zero
or divide one externally governed allowance across processes. The integrated
V3 acquisition service uses its PostgreSQL `fulltext_budget` ledger as the sole
shared paid-provider scheduler.
...
For parallel discovery, assign adjacent `(start, end]` ID ranges. Each range
has a distinct persisted watermark. `--max-batches 0` drains the assigned
range, and the final open-ended range picks up new source IDs on its next run:

```bash
python -m src.corpus.niche.discover --id-start 0       --id-end 1609098 --max-batches 0
python -m src.corpus.niche.discover --id-start 1609098 --id-end 3218196 --max-batches 0
python -m src.corpus.niche.discover --id-start 3218196 --id-end 4827294 --max-batches 0
python -m src.corpus.niche.discover --id-start 4827294 --id-end 0       --max-batches 0
```

Never overlap the bounds. The lower bound is exclusive and the upper bound is
inclusive, so the four commands cover every positive source ID exactly once.
