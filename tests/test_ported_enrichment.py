...
def test_grounded_threshold_matches_webapp():
    ...
    Asserts the INVARIANT (one shared threshold, one shared implementation) rather than a literal.
    This previously pinned 0.6, which meant the test passed happily while the rule it guarded had
    silently stopped working: that 60% bag-of-words check got easier as reference passages grew,
    and measured rationale overclaim went 10% -> 26.3% after a full-text backfill without this
    test ever going red. Pinning the number tested that nobody edited a constant; it did not test
    that the grounding worked.
    """
    import grounding
    assert claim_chart.MIN_OVERLAP == grounding.MIN_SPAN
    assert claim_chart._grounded.__module__ == "claim_chart"
    # both surfaces must resolve to the same underlying check
    quote, src = "a sealing lip arranged circumferentially", "the sealing lip is arranged circumferentially about the rim"
    assert claim_chart._grounded(quote, src) == grounding.grounded(quote, src)


def test_grounding_does_not_weaken_as_passages_grow():
    """The regression that motivated grounding.py: appending unrelated text to a reference must
    NOT turn an ungrounded quote into a grounded one. The old global-overlap rule failed this."""
    import grounding
    short = "A vacuum gripper having a suction cup and a pump."
    fabricated = "a capacitive sensor detects partial loss of vacuum and alarms the operator"
    padding = (" The device comprises a housing. A capacitive element may be provided. "
               "Loss of pressure is detected by a switch. An operator alarm sounds. "
               "The sensor monitors the seal. Vacuum is generated by the pump. ") * 40
    assert grounding.grounded(fabricated, short) is False
    assert grounding.grounded(fabricated, short + padding) is False, \
        "grounding weakened as the passage grew — the exact post-backfill regression"
    # a genuine verbatim quote still passes in both the short and the padded case
    assert grounding.grounded("a suction cup and a pump", short) is True
    assert grounding.grounded("a suction cup and a pump", short + padding) is True
