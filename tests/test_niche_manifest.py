...
def test_discovery_ranges_have_disjoint_restart_watermarks_and_hard_ends():
    queries = []
    ...
    left = LocalDiscoverySource(
        Connection,
        id_window=100,
        range_start=1_000,
        range_end=1_150,
    )
    right = LocalDiscoverySource(
        Connection,
        id_window=100,
        range_start=1_150,
        range_end=1_300,
    ...
    _, advanced = left.seed_records(
        DOMAIN_GROUPS,
        {left.watermark_key: 1_050},
        10,
    )
    left_range_query = next(
        params for statement, params in queries if "p.id > %s AND p.id <= %s" in statement
    )

    assert left.watermark_key != right.watermark_key
    assert left_range_query[:2] == (1_050, 1_150)
    assert advanced[left.watermark_key] == 1_150
    assert advanced[left.source_max_watermark_key] == 1_150


def test_discovery_range_rejects_an_empty_or_reversed_interval():
    try:
        LocalDiscoverySource(lambda: None, range_start=1_000, range_end=1_000)
    except ValueError as exc:
        assert "range end" in str(exc).lower()
    else:
        raise AssertionError("empty discovery range was accepted")
