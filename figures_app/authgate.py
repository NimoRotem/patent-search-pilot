...
password of its own and never will, for the same reason the register-lookup engine has none —
a second credential for the same person on the same domain is a thing to lose, not a security
measure.
...
* The search app signs a Flask session cookie with the key in ``~/patent-search-pilot/.secret_key``
  at path ``/``, so the browser sends it here too. We verify that signature with the same key.
  A forged cookie cannot get in and no password is stored on this side.
* A valid signature is necessary but not sufficient. The cookie lasts thirty days and carries
  ``session_version``, which the search app bumps on a password change; we check it against the
  account row, so deactivating a user or resetting their password closes this app to them too.
...
_signer = Flask("pfc-authgate-signer")
...
def require_user(view):
    ...
    def wrapper(*args, **kwargs):
        user = request.environ.get("pfc.user")
        if user is None:
...
def user() -> dict:
    return request.environ.get("pfc.user") or {}
