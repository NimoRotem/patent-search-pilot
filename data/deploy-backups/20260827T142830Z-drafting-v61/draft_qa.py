...
REVIEW_PROMPT = """Review this patent application.

Every text file is below: the inventor's disclosure and conversation, all ten sections, the numeral
ledger, and every figure brief. Review ALL of it, not a sample.

%(materials)s

These are NOT above and are still yours to read from the workspace when a finding turns on them:
  figures/rendered-*.png              the actual rendered sheets
  review/figure-audit-evidence.json   exact-image OCR, geometry, leader and endpoint evidence
  prior_art/                          the references the draft cites, with their real text
Compare the exact pixels and the prior audit evidence with the brief and the patent text above.
Read a prior_art file when you are judging what the draft SAYS about that reference; do not read
them all as a matter of routine.
...
REVIEW_PROMPT_FETCH = """Review the draft in this workspace.
...
#  Two prompts, because there are two ways to give a reviewer 16,000 tokens of material, and one
#  of them costs 458,000. See draft_workspace.material_block: the whole workspace is handed over
#  in the prompt when it fits, and only a workspace too large for that falls back to a reviewer
#  that fetches its own files. Both say the same thing about WHAT to review.
SOURCE_REVIEW_PROMPT = """Run the source-fidelity preflight identified as
%(version)s.

The complete material is below. It is the whole of what you are reviewing: every input file, every
section of the draft, the numeral ledger, and every figure brief. Nothing is withheld and nothing
is abbreviated, so there is nothing to go and fetch. Review ALL of it, not a sample.

%(materials)s

Ignore rendered image files; they are not part of this review. Return the complete structured
review. When finished, call
...
SOURCE_REVIEW_PROMPT_FETCH = """Run the source-fidelity preflight identified as
%(version)s.

This workspace is too large to hand you in one piece, so read these files in full yourself:
  input/disclosure.md
...
use at least 120 characters and explicitly state that the complete claims, numerals or numbered
parts, figure briefs or drawings, and affirmative inventor sources were all checked and traced.
Never omit the findings property, even when there is nothing to report. Keep the summary below
8,000 characters."""
...
def review_sources(workspace: Path, *, transcript: Path | None = None, model: str = "",
    ...
    #  Hand it the material. This review is pure text - its own prompt says to ignore the rendered
    #  images - so when the workspace fits there is nothing it needs a file tool for, and a
    #  reviewer with no file tools cannot spend twenty-seven model turns fetching sixteen files.
    materials, covered = draft_workspace.material_block(
        workspace, draft_workspace.MATERIAL_DIRECTORIES)
    if materials:
        prompt = SOURCE_REVIEW_PROMPT % {"version": SOURCE_REVIEW_VERSION,
                                         "materials": materials}
        tools, allowed_bash = "", ()
    else:
        prompt = SOURCE_REVIEW_PROMPT_FETCH % {"version": SOURCE_REVIEW_VERSION}
        tools, allowed_bash = "Read,Glob,Grep", ()
    quality_guidance = ""
    for quality_attempt in range(2):
        try:
            run = draft_agent.run(
                workspace=workspace,
                prompt=prompt + quality_guidance,
        ...
        quality_guidance = (
            ...
            quality_error + " Review " +
            ("every part of the material above again" if materials
             else "every required file again") +
            " and return a substantive summary that explicitly evidences review of claims, "
            "numerals, figures, and inventor sources. Do not repeat the prior short summary."
        ) if quality_error else ""
...
def review(workspace: Path, *, checks: Sequence[Mapping[str, Any]],
    ...
    #  Same trade as review_sources, with one difference: this reviewer also looks at the RENDERED
    #  sheets and quotes the prior art it is judging a characterisation of, and neither of those
    #  can travel in a prompt. So it keeps its file tools and is simply spared the fifteen text
    #  reads it used to spend them on.
    materials, covered = draft_workspace.material_block(
        workspace, draft_workspace.MATERIAL_DIRECTORIES)
    prompt = (REVIEW_PROMPT % {"checks": context, "materials": materials} if materials
              else REVIEW_PROMPT_FETCH % {"checks": context})
    try:
        run = draft_agent.run(
            workspace=workspace,
            prompt=prompt,
