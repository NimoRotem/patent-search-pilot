...
def test_valid_candidate_is_checkpointed_before_the_long_review(monkeypatch, tmp_path):
    ...
    #  The long part of a turn is the independent review now: a drafting turn does not draw.
    monkeypatch.setattr(
        runner, "evaluate",
...
def test_transient_drawing_capacity_is_reported_not_raised(monkeypatch, tmp_path):
    """The image provider being out of capacity is not a defect in the application, and the
    drawing pass is not allowed to fail anything: it says what it did not finish."""
    ...
    faults = runner._reconcile_drawings(
        turn_id=3, lease="lease", project_id=7, user_id=91, sections=GOOD,
        numerals=NUMERALS, figures=FIGURES, disclosure="d", workspace=tmp_path)

    assert faults and "RESOURCE_EXHAUSTED" in faults[0]


def test_a_drawing_defect_is_reported_and_never_holds_up_the_text(monkeypatch, tmp_path):
    """A drawing failure returns from the pass rather than raising, and the report it produces
    still refuses a filing package."""
    runner = draft_studio.TurnRunner(Mock(), object(), agent=Mock(), workspace=Mock())
    monkeypatch.setattr(runner, "_drawings_already_match", lambda *_a, **_k: False)
    monkeypatch.setattr(runner, "_ensure_figures", lambda **_k: {
        "ok": False, "errors": ["FIG. 2: wrong fastener axis", "FIG. 7: missing process arrow"]})

    faults = runner._reconcile_drawings(
        turn_id=3, lease="lease", project_id=7, user_id=91, sections=GOOD,
        numerals=NUMERALS, figures=FIGURES, disclosure="d", workspace=tmp_path)

    assert faults == ["FIG. 2: wrong fastener axis", "FIG. 7: missing process arrow"]
    report = {"status": "complete", "checks": [
        {"name": draft_studio._DRAWING_INSPECTION_CHECK, "status": "fail",
         "category": "figures_and_numerals", "items": faults}], "findings": []}
...
def test_a_figure_plan_defect_is_reported_and_the_text_publishes_intact(
        monkeypatch, tmp_path):
    monkeypatch.setattr(draft_figures, "discard_project_figure_checkpoint",
                        lambda _turn_id: False)
    repository = Mock()
    ...
    #  A figure-plan defect is a defect in the DRAWINGS, so it is reported and the text publishes
    #  on its own merits: one agent call, not two. The source lock that stops a figure-plan repair
    #  erasing filing text is still exercised directly, above, and on the resuming-candidate path.
