...
HOW TO WORK
READ WHAT THE REQUEST NEEDS, NOT THE WHOLE WORKSPACE. Always read input/request.md first, then open
only what it actually bears on. A change to one section needs that section, draft/numerals.md and
the disclosure passage behind it; it does not need every reference in prior_art/ or every other
section. Reading a file you are not going to use costs the same as reading one you are.
Use Grep to find where a term or numeral is used rather than reading files to look for it.
Then edit only what the request and the review require. A request to narrow one claim is
...
READ ONLY WHAT YOU NEED. Start with review/previous-qa.md. It names every defect and the section
each one is in: open those files and the numeral table, and nothing else unless a finding sends you
there. You are a fresh session with the whole workspace on disk, so re-reading files that no
finding mentions costs the turn and tells you nothing you are about to change.

Then fix every listed mechanical check and every independently
...
#  The lane the user types into, so this is the one whose SPEED is felt directly. It used to open
#  with a reading list - the rest of draft/, the disclosure, the conversation, the prior-art index
#  - which is eight or nine tool calls before a word is written, and every one of them another
#  model turn over everything read so far. The whole application is 16,000 tokens; it goes in the
#  prompt.
SECTION_EDIT_PROMPT = """Edit ONE section of this application: %(heading)s (the file
draft/%(filename)s).

Everything you need is below: the inventor's disclosure and conversation, the user's request, the
figure briefs and every section of the draft. Read all of it before you change anything, because
a clause that contradicts claim 1 is worse than no clause.

%(materials)s

===== BEGIN what the user asked for =====
%(request)s
===== END what the user asked for =====

Return the smallest set of find/replace pairs inside draft/%(filename)s that does what was asked.
Change nothing else and touch no file: your structured answer is the only output. A `find` value
must be copied verbatim out of the %(heading)s block above, and must be long enough to appear
exactly once in it.

The prior-art lookup is the one thing not above: run tools/patent_lookup.py when you need what a
reference ACTUALLY says. Never characterise a reference from memory."""

SECTION_EDIT_PROMPT_FETCH = """Edit ONE section of this application: %(heading)s (the file
draft/%(filename)s).

What the user asked for is in input/request.md.

This workspace is too large to hand you in one piece. Read before you write anything:
input/request.md, draft/%(filename)s, draft/numerals.md, the rest of draft/, input/disclosure.md,
input/conversation.md, and prior_art/INDEX.md with any reference that bears on the request. Use
tools/patent_lookup.py if you need what a reference actually says.
...
# =============================================================================================
# The repair round: a patch, not a rewrite
# =============================================================================================
#  A REPAIR ROUND USED TO REWRITE THE WHOLE APPLICATION TO FIX TWO SENTENCES.
#  Measured on project 8, turn 58: one repair run took 12 internal model turns and put 10.5 MILLION
#  tokens through the model to produce 5,213. It was told to "read only what you need", which is
#  advice, not a constraint, and advice loses to an agent that would rather look at everything. And
#  every one of those reads is another model turn, and every model turn re-reads the entire
#  conversation so far, which is where a ten-million-token repair of a 38,000-character draft
#  comes from.
#
#  So the constraint is now structural rather than advisory, and it is the owner's own instruction
#  from the first turn of this work: send the context, ask for the DIFF, apply it here. The round
#  is handed the complete material and the gate report, has NO TOOLS AT ALL, and the only thing it
#  can return is a list of exact find/replace pairs against named files. It cannot spend a turn
#  reading, it cannot rewrite a section it was not asked about, and what it did change is legible
#  in the response instead of having to be diffed out of the filesystem afterwards.
#
#  Keyed on file PATH rather than section key on purpose: a repair legitimately touches
#  draft/numerals.md and the figures/ briefs, and neither of those is a section.
REPAIR_SCHEMA = {
    "type": "object",
    "properties": {
        "action": {"type": "string", "enum": ["revised"]},
        "summary": {"type": "string"},
        "changes": {"type": "array", "items": {"type": "string"}},
        "patches": {
            "type": "array",
    ...
    "required": ["action", "summary", "changes", "patches", "rewrites"],
    "additionalProperties": False,
}

REPAIR_SYSTEM = """You are repairing a US utility patent application that did not pass its
automatic filing gate. You have the complete application, the inventor's sources, the figure
briefs and the gate report in front of you. There is nothing else to consult and no tool to
consult it with.

Return the CHANGES, not the document. Every change is either an exact find/replace pair against
one named file, or, when a section genuinely has to be rebuilt, a whole replacement for that one
file. Quote the `find` text exactly as it appears, including punctuation, and quote enough of it
to be unique within that file. Text you do not name is kept byte for byte, which is the point:
a repair that rewrites paragraphs nothing was wrong with is how a checked draft loses the wording
somebody already approved.

Fix every listed mechanical check and every verified finding. Do not argue with a finding. If
wording triggered a false positive, make the draft unambiguous enough that the check passes
without changing the invention.

Generated pixels are never authority for the invention. The authority order is the inventor's
disclosure and conversation, then the patent text, then the figure briefs, then the rendered
sheets. Never change the claims, description, numeral table, or disclosed embodiments merely to
excuse something the image model drew incorrectly, and never add an implementation detail or an
embodiment because it appeared in a generated image.

Never patch or rewrite a rendered PNG, SVG, JPEG, audit-evidence file, or any other generated
artifact. A drawing defect is repaired only by changing the authoritative Markdown figure brief
and letting the drawing pipeline render and inspect fresh pixels automatically.

Keep the numeral table, the drawing descriptions, the detailed description, the claims and the
figure briefs synchronized: if a change to one of them requires a change to another, patch both
in the same answer. Leave no note, placeholder, question or instruction for a person anywhere in
the application text."""

REPAIR_PROMPT = """The current draft did not pass the automatic filing gate.

%(materials)s

===== BEGIN the gate report =====
%(report)s
===== END the gate report =====

Fix every defect named in the report. Return `patches` and, only where a file must be rebuilt,
`rewrites`. Set `action` to "revised" and list what you changed in `changes`."""


#  A REVISION TURN IS A PATCH TOO.
#  This is the lane the user actually types into: "make claim 1 broader", "the background is too
#  long". It ran the same machine as a first draft - full tool set, whole application rewritten,
#  the reviewer then reading all of it back. Measured on project 8, turn 58: the drafting run
#  alone opened 863,866 tokens of context to return 90 tokens of answer.
#
#  Nothing about a revision needs that. The application is 16,000 tokens and it fits in the
#  prompt; what comes back should be the change, not the document. The same schema as a repair,
#  plus the one thing a repair never does: ANSWER. A question about the draft is not a change to
#  it, and a turn that answers must not touch a file.
REVISION_SCHEMA = {
    "type": "object",
    "properties": {
        "action": {"type": "string", "enum": ["revised", "answered"]},
        ...
        "changes": {"type": "array", "items": {"type": "string"}},
        "consequences": {"type": "array", "items": {"type": "string"}},
        #  Kept from the full turn schema on purpose. `reasoning` is not: the per-change `why`
        #  already says why, and paying for it twice is what this whole lane exists to stop. How
        #  the draft distinguishes the art is a different thing, and a revision that touches the
        #  claims can change it.
        "prior_art_strategy": {"type": "string"},
        "patches": {
            ...
            "items": {
                ...
                "properties": {"path": {"type": "string"}, "find": {"type": "string"},
                               "replace": {"type": "string"}, "why": {"type": "string"}},
                "required": ["path", "find", "replace", "why"], "additionalProperties": False}},
        "rewrites": {
            ...
            "items": {
                ...
                "properties": {"path": {"type": "string"}, "text": {"type": "string"},
                               "why": {"type": "string"}},
                "required": ["path", "text", "why"], "additionalProperties": False}},
    },
    "required": ["action", "summary", "answer", "changes", "consequences",
                 "prior_art_strategy", "patches", "rewrites"],
    "additionalProperties": False,
}

REVISION_SYSTEM = """You are revising a US utility patent application that is already drafted and
already checked. The complete application, the inventor's sources and the figure briefs are in
front of you. There is nothing else to consult except the prior-art lookup tool.

Return the CHANGES, not the document. Every change is either an exact find/replace pair against
one named file, or, where a section genuinely has to be rebuilt, a whole replacement for that one
file. Quote the `find` text exactly as it appears, including punctuation, and quote enough of it
to be unique within that file. Everything you do not name is kept byte for byte, and that is the
point: this draft has been read and approved by a person, and a revision that rewrites paragraphs
nobody asked about throws that away silently.

Change only what the request requires, plus whatever else must move to keep the application
consistent: the claims, the detailed description, the numeral table and the figure briefs have to
agree with each other, so if the request moves one of them, patch the others in the same answer.

If the request is a QUESTION rather than a change, answer it in `answer`, set `action` to
"answered", and return no patches and no rewrites at all. If answering honestly would require a
change to the draft, say so in the answer and ask them to confirm; do not make the change unasked.

If a review finding is genuinely mistaken, leave the draft alone and say why in your summary
rather than changing good text to silence a check. Leave no note, placeholder, question or
instruction for a person anywhere in the application text."""

REVISION_PROMPT = """Revise this patent application.
...
===== END what the user asked for =====
%(report)s
Do what was asked. Return `patches`, and `rewrites` only where a file must be rebuilt. Set
`action` to "revised" and list what you changed in `changes`. If this is a question rather than a
change, set `action` to "answered" instead and return no patches."""

_REVISION_REPORT_BLOCK = """
===== BEGIN the last review of this draft =====
%s
===== END the last review of this draft =====
Fix anything in that review that is genuinely wrong. If you think a finding is mistaken, leave the
draft alone and say why.
"""


class RepairPatchError(StudioError):
    """A repair patch that does not fit the draft it claims to patch."""


#  Only these. A repair has no business writing to input/, prior_art/ or review/, and a path that
#  escapes the workspace is not a typo worth being tolerant about.
REPAIR_WRITABLE = ("draft/", "figures/")
_NEW_FIGURE_REWRITE_RE = re.compile(
    r"figures/FIG-[1-9][0-9]{0,2}[A-Za-z]?\.md", re.IGNORECASE)


def apply_repair_patches(workspace: Path, result: Mapping[str, Any]) -> list[str]:
    """Apply a repair round's patches to the workspace, or refuse the whole set and say why.

    ALL OR NOTHING, deliberately. Applying eleven of twelve patches would leave a draft that no
    reviewer has read and no agent intended, and the twelfth failure would be discovered at the
    next gate as something that looks like a fresh defect. A patch that does not fit is a repair
    that missed, and the honest thing is to run the round again with the miss quoted back.
    """
    root = Path(workspace)
    pending: dict[str, str] = {}

    def resolve(raw: Any, index: int, kind: str) -> tuple[str, Path]:
        path = str(raw or "").strip().lstrip("./")
        if not path.startswith(REPAIR_WRITABLE):
            raise RepairPatchError(
                f"{kind.title()} {index} names {path[:120]!r}, which a repair may not write. "
                f"A repair writes only to {' and '.join(REPAIR_WRITABLE)}")
        if path.startswith("figures/") and not path.casefold().endswith(".md"):
            raise RepairPatchError(
                f"{kind.title()} {index} names generated drawing artifact {path!r}. "
                "Repair its authoritative Markdown figure brief instead.")
        unresolved = root / path
        target = unresolved.resolve()
        if not str(target).startswith(str(root.resolve()) + os.sep):
            raise RepairPatchError(f"{kind.title()} {index} names a path outside the workspace.")
        may_create_figure = (
            kind == "rewrite"
            and not unresolved.exists()
            and not unresolved.is_symlink()
            and bool(_NEW_FIGURE_REWRITE_RE.fullmatch(path))
            and target.parent == (root / "figures").resolve()
        )
        if not target.is_file() and not may_create_figure:
            raise RepairPatchError(
                f"{kind.title()} {index} names {path!r}, which is not a file in this workspace.")
        return path, target

    for index, item in enumerate((result.get("rewrites") or ()), 1):
        if not isinstance(item, Mapping):
            continue
        path, target = resolve(item.get("path"), index, "rewrite")
        text = str(human_text(item.get("text") or "")).replace("\x00", "").strip()
        if not text:
            raise RepairPatchError(f"Rewrite {index} would empty {path!r}.")
        pending[path] = text

    for index, item in enumerate((result.get("patches") or ()), 1):
        if not isinstance(item, Mapping):
            continue
        path, target = resolve(item.get("path"), index, "patch")
        if path not in pending:
            pending[path] = target.read_text(encoding="utf-8")
        text = pending[path]
        find = str(human_text(item.get("find") or ""))
        if not find.strip():
            raise RepairPatchError(f"Patch {index} has nothing to find.")
        replace = str(human_text(item.get("replace") or ""))
        occurrences = text.count(find)
        if occurrences == 1:
            pending[path] = text.replace(find, replace, 1)
            continue
        if occurrences == 0:
            #  Whitespace is the usual near miss: a wrapped paragraph quoted back with its line
            #  breaks collapsed. Accept that reading once, and only when it is unambiguous.
            loose = _loose_find(text, find)
            if loose is None:
                raise RepairPatchError(
                    f"Patch {index} looks for text that is not in {path}: {find[:160]!r}.")
            start, end = loose
            pending[path] = text[:start] + replace + text[end:]
            continue
        raise RepairPatchError(
            f"Patch {index} looks for text that appears {occurrences} times in {path}, so where "
            f"it belongs is ambiguous: {find[:160]!r}.")

    if not pending:
        raise RepairPatchError("The repair round returned no change to apply.")
    for path, text in pending.items():
        (root / path).write_text(
            str(human_text(text)).replace("\x00", "").rstrip() + "\n", encoding="utf-8")
    return sorted(pending)


def build_revision_prompt(workspace: Path,
                          report: Mapping[str, Any] | None = None) -> tuple[str, bool]:
    """The revision prompt and whether the material travelled with it.

    False means the workspace was too large to hand over and the caller must keep the old
    tool-driven revision. It is never a quietly shortened version of the same prompt.
    """
    materials, _covered = draft_workspace.material_block(workspace)
    if not materials:
        return "", False
    request_path = Path(workspace) / "input" / "request.md"
    try:
    ...
    previous = ""
    if report:
        previous = _REVISION_REPORT_BLOCK % _report_lines(report)
    return REVISION_PROMPT % {
        "materials": materials,
        "request": request or "(no request was recorded for this turn)",
        "report": previous}, True


def _report_lines(report: Mapping[str, Any]) -> str:
    """A gate report as the list of things still wrong, and nothing else.

    The checks that PASSED are left out on purpose. There were 40-odd of them and repeating them
    to the model is paying to be told what is already fine.
    """
    lines = []
    for check in (report.get("checks") or ()):
        ...
        for item in (check.get("items") or ())[:12]:
            lines.append(f"    . {item}")
    for finding in (report.get("findings") or ()):
        lines.append(f"- FINDING {finding.get('title')} ({finding.get('where')})")
        for key in ("detail", "evidence", "fix"):
            if finding.get(key):
                lines.append(f"    {key}: {finding.get(key)}")
    return "\n".join(lines) or str(report.get("summary") or "The gate did not pass.")


def build_repair_prompt(workspace: Path, report: Mapping[str, Any]) -> tuple[str, bool]:
    """The repair prompt and whether the material travelled with it.

    False means this workspace was too large to hand over and the caller must keep the old
    tool-driven repair. It is never a quietly shortened version of the same prompt.
    """
    materials, _covered = draft_workspace.material_block(workspace)
    if not materials:
        return "", False
    return REPAIR_PROMPT % {"materials": materials, "report": _report_lines(report)}, True


def _patch_miss_report(exc: Exception) -> dict[str, Any]:
    """A repair whose patch did not fit, expressed as the gate failure it is."""
    check = {"name": "The repair patch applies to the current draft",
             "status": "fail", "severity": "error", "category": "internal_logic",
             "detail": str(exc)[:1200],
             "items": [str(exc)[:600],
                       "Quote the text to find exactly as it appears in the named file, "
                       "including punctuation and line breaks, and quote enough of it to be "
                       "unique within that file."]}
    return {"status": "failed", "verdict": "fail",
            "summary": "The repair did not apply: " + str(exc)[:1000],
            "checks": [check], "findings": [],
            "counts": draft_qa.counts_for([check], []), "cost_usd": 0.0,
            "duration_ms": 0, "model_name": "", "last_error": str(exc)[:1200]}


def build_section_edit_prompt(section_key: str, sections: Mapping[str, str],
                              workspace: Path | None = None) -> tuple[str, bool]:
    """The section-edit prompt, and whether the material travelled with it."""
    ...
    if workspace is not None:
        materials, _covered = draft_workspace.material_block(workspace)
    if not materials:
        return SECTION_EDIT_PROMPT_FETCH % {
            "heading": heading, "filename": filename, "current": current}, False
    request_path = Path(workspace) / "input" / "request.md"
    try:
        request = request_path.read_text(encoding="utf-8").strip()
    except OSError:
    ...
    return SECTION_EDIT_PROMPT % {
        "heading": heading, "filename": filename, "materials": materials,
        "request": request or "(no request was recorded for this turn)"}, True
...
#  AN ADVISORY DOES NOT BLOCK. It is reported, and that is what it is for.
#  Three checks are written as advisories on purpose - "Antecedent basis in the claims", "Claim
#  terms appear in the description", "Numerals are introduced with their part name" - because each
#  one is a heuristic that false-positives on correct drafting. Their own text says so: "check
#  whether the description needs to be widened OR the claim reworded". They were blocking anyway,
#  and the cost of that is not just the wasted rounds. The repair prompt says "fix every listed
#  mechanical check and do not argue with a finding", so a blocking advisory makes the agent
#  CHANGE CORRECT CLAIM LANGUAGE to satisfy a regex, six rounds deep, with two reviews between
#  each. That is the most expensive thing this system can be asked to do and it makes the draft
#  worse. Anything that must actually stop a filing is severity error or warn, not advisory.
_ADVISORY = "advisory"
...
def _blockers(report: Mapping[str, Any], *, drawings: bool, text: bool) -> list[str]:
    ...
    for check in report.get("checks") or ():
        if str(check.get("status") or "") == "pass":
            continue
        if str(check.get("severity") or "") == _ADVISORY:
            continue
    ...
    for finding in report.get("findings") or ():
        if str(finding.get("severity") or "") == _ADVISORY:
            continue
...
class TurnRunner:
    ...
    def _run_agent(self, *, turn_id: int, lease: str, workspace: Path, prompt: str,
                   ...
                   stage: str, model: str = "", system_prompt: str | None = None,
                   schema: Mapping[str, Any] | None = None,
                   tools: str | None = None, house: str = "") -> draft_agent.AgentRun:
        ...
        #  None means "the drafting default"; an EMPTY STRING for tools means no tools at all, and
        #  those are different things. Conflating them is why a run meant to hold its material and
        #  answer got the full Read/Write/Edit/Glob/Grep/Bash set and went looking anyway.
        ...
        try:
            ...
            if tools == "":
                #  A run with no tools must not be handed a Bash allow-list either, or the CLI is
                #  still told there is a command it may reach for.
                extra["allowed_bash"] = ()
            ...
            run = self.agent.run(
                ...
                system_prompt=(DRAFT_SYSTEM if system_prompt is None else system_prompt)
                + str(house or ""),
                schema=TURN_SCHEMA if schema is None else schema,
    ...
    def _reconcile_drawings(self, *, turn_id: int, lease: str, project_id: int, user_id: int,
        ...
        Returns what is still wrong rather than raising it. The sheets are still filing artifacts
        and `filing_blockers` still refuses a package while any of this is outstanding; what has
        changed is that the TEXT no longer waits on it.
        """
        import draft_figures
    ...
    def _drawings_already_match(project_id: int, user_id: int,
        ...
        Deliberately asks the STORED audits rather than today's gate. `current_semantic_audit` and
        its siblings answer "was this checked by the gate we ship now", which is a different and
        much stricter question: one bumped constant makes it False for every drawing ever taken and
        sends the whole corpus back through the pipeline. What matters here is whether these pixels
        were ever verified against this exact specification, and whether that verdict was a pass.
        ...
        try:
            ...
            for spec in specs:
                ...
                for key in ("semantic_audit", "leader_audit"):
                    audit = active.get(key) or {}
                    if not audit.get("ok") or audit.get("specification_hash") != want:
                        return False
                ocr = active.get("numeral_audit") or {}
                if not ocr.get("ok") or not ocr.get("inspected"):
                    return False
            return True
    ...
    def _run_section_edit(self, turn: Mapping[str, Any], context: Mapping[str, Any]
        ...
        edit_prompt, edit_patched = build_section_edit_prompt(
            section_key, sections, workspace)
        run = self._run_agent(
            turn_id=turn_id, lease=lease, workspace=workspace, prompt=edit_prompt,
            ...
            #  No Write and no Edit either way. The tool set is what makes this a patch rather
            #  than a rewrite: the agent cannot change the workspace, so its structured answer is
            #  the only output there is, and it is small. When it is handed the material it keeps
            #  only Bash, for the prior-art lookup, which is the one thing not in the prompt.
            tools="Bash" if edit_patched else "Read,Glob,Grep,Bash")
    ...
    def run(self, turn: Mapping[str, Any]) -> dict[str, Any]:
        ...
        #  EVERY RUN STARTS A FRESH SESSION, AND THAT IS THE WHOLE ECONOMY OF THIS AGENT.
        #
        #  It used to resume the PROJECT's session, so one Claude Code conversation accumulated
        #  across every turn the project had ever had. Measured on turn 55 of project 8: the first
        #  run of the turn carried 546,614 tokens of inherited transcript BEFORE it read a single
        #  file, and by run 31 that had grown to 771,916. The whole workspace is 45,000 tokens.
        #  The turn read 192 MILLION tokens to write 162 thousand: 1,183 read per token produced.
        #
        #  Nothing of value was in that transcript that is not already on disk in a better form.
        #  The workspace IS the state - the files are the draft - and continuity is carried by
        #  input/conversation.md, review/previous-qa.md and the draft itself, each of them curated.
        #  A raw transcript of every tool call the agent ever made is the same information at a
        #  hundred times the price, and it can only grow.
        prior_session = ""
        #  A FIRST DRAFT WRITES FILES. A REVISION RETURNS A PATCH.
        #  Writing ten sections that do not exist yet is exactly the job the file tools are for.
        #  Changing a clause in a checked application is not, and running the same machine for
        #  both is what made "make claim 1 broader" cost what a first draft costs: the drafting
        #  run of turn 58 opened 863,866 tokens of context to return 90 tokens of answer.
        revising = not first and prompt_kind in ("revise", "question")
        revision_prompt, patched_revision = ("", False)
        if revising:
            revision_prompt, patched_revision = build_revision_prompt(
                workspace, context.get("prepared_qa") or {})
        run = _gate_resume_run(context, turn)
        if run is None:
            try:
                run = self._run_agent(
                    turn_id=turn_id, lease=lease, workspace=workspace,
                    prompt=revision_prompt if patched_revision else prompt,
                    ...
                    schema=REVISION_SCHEMA if patched_revision else None,
                    system_prompt=REVISION_SYSTEM if patched_revision else None,
                    #  The lookup tool stays: what a cited reference ACTUALLY says is the one
                    #  thing that is not in the prompt and must never be guessed at.
                    tools="Bash" if patched_revision else None,
                    house=draft_settings.prompt_additions(self._settings_for(project)))
            except StudioError as exc:
                self._checkpoint_interrupted_agent(
                    turn_id=turn_id, lease=lease, workspace=workspace, allowed=allowed, error=exc)
                raise
            if patched_revision and str(
                    (run.result or {}).get("action") or "revised") != "answered":
                #  Same all-or-nothing rule as a repair: a patch that misses is a revision that
                #  missed, and the user is told so rather than being handed a half-applied change.
                try:
                    touched = apply_repair_patches(workspace, run.result or {})
                except RepairPatchError as exc:
                    #  StudioError takes no keyword arguments; `category` is an optional attribute
                    #  the gate reads off the exception with getattr.
                    failure = StudioError(
                        f"The revision did not apply to the draft. {exc} Ask again and it will "
                        f"be retried against the current text.")
                    failure.category = "internal_logic"
                    raise failure from exc
                run.result = dict(run.result or {})
                run.result["changes"] = list(run.result.get("changes") or []) + [
                    "Applied the revision to " + ", ".join(touched) + "."]
        else:
            self.repository.heartbeat(
        ...
        patch_miss: dict[str, Any] | None = None
        for review_index in range(rounds_allowed):
            if review_index:
                prior_snapshot, prior_report = snapshot, report or {}
                #  A repair round is handed the draft and the review that judged it, and returns
                #  the DIFF. It has no tools, so it cannot spend twelve model turns re-reading a
                #  workspace it is already holding, and it cannot rewrite a paragraph no finding
                #  mentioned. Resuming the previous round would be worse still: that carries its
                #  whole tool-call transcript forward, six times over.
                repair_prompt, patched = build_repair_prompt(workspace, prior_report)
                try:
                    repair = self._run_agent(
                        turn_id=turn_id, lease=lease, workspace=workspace,
                        prompt=repair_prompt if patched else FINALIZE_PROMPT,
                        session_id=self.agent.new_session_id(),
                        resume=False, transcript=transcript, stage="repairing the draft",
                        model=self._model_for(project),
                        schema=REPAIR_SCHEMA if patched else None,
                        system_prompt=REPAIR_SYSTEM if patched else None,
                        tools="" if patched else None,
                ...
                if patched:
                    #  A patch that does not fit is a repair that MISSED, and it is fed back as a
                    #  gate failure so the next round sees the miss quoted. Applying the ones that
                    #  did fit would publish a draft nobody wrote and nobody read back.
                    try:
                        touched = apply_repair_patches(workspace, result)
                    except RepairPatchError as exc:
                        #  The workspace is untouched, so `snapshot` and `sections` from the last
                        #  round are still exactly true and must be LEFT ALONE. Blanking them here
                        #  is the same defect the source-lock comment above warns about: an empty
                        #  snapshot lets the next round's restore erase every filing section.
                        patch_miss = _patch_miss_report(exc)
                    else:
                        result["changes"] = list(result.get("changes") or []) + [
                            "Applied the repair to " + ", ".join(touched) + "."]
                action = "revised"
                source_lock = restore_text_after_drawing_only_review(
            ...
            if patch_miss is not None:
                #  Nothing changed on disk, so there is nothing new to review. The miss goes round
                #  as the gate failure it is, with the text it looked for quoted back.
                report, patch_miss = patch_miss, None
            elif answered_without_candidate and review_index == 0:
                ...
                try:
                    ...
                    # Text drafting and drawing generation are separate bounded turns. The first
                    # turn publishes source-clean text and atomically queues the second. The
                    # automatic continuation owns image generation, pixel audits, and the final
                    # independent package review, so nobody has to click or intervene.
                    self.repository.heartbeat(
                    ...
                    if drawing_continuation:
                        ...
                        report = self.evaluate(
                            ...
                            numerals=snapshot["numerals"], figures=snapshot["figures"],
                            review_index=review_index, turn_id=turn_id, lease=lease,
                            review_model=self._settings_for(project).get("review_model") or "")
                    else:
                        # The final reviewer must open every rendered sheet. Before the separate
                        # drawing turn, use the source reviewer plus deterministic text checks.
            ...
            # Text publishes on text merits. The automatic drawing continuation is the complete
            # filing gate and cannot publish around a drawing or final-review defect.
