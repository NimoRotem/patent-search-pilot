...
#  How long one turn may spend bringing the drawings into line with the text before it publishes
#  the text anyway and reports what is still wrong. Measured on a real project, an unbounded pass
#  spent 53 minutes of model time across 190 inspection calls on sheets whose specification had not
#  changed, because a bumped audit version invalidates every stored audit and the gate fails closed.
...
class DrawingBudgetSpent(StudioError):
    """The drawing pass ran out of its budget. Not a defect in the draft, and never fatal."""
...
def text_blockers(report: Mapping[str, Any]) -> list[str]:
    """The subset a person cannot go on drafting through.

    THE DRAWINGS ARE NOT THE APPLICATION'S TEXT AND MUST NOT HOLD IT HOSTAGE. A drawing gate that
    blocks publication makes every wording change wait for the image pipeline, which on a real
    project meant one hour and thirty-eight minutes and a hundred and ninety inspection calls for a
    one-sentence change to the Field of the Disclosure. Worse, it fails CLOSED on a bumped audit
    version: sheets that already passed for exactly this specification are re-examined from scratch
    on every turn for ever, so the wait never gets shorter by making progress.

    So the text publishes on the text's own merits. The drawing state is recorded, shown in
    Drawings, and still refuses to hand anybody a filing package: `filing_blockers` is unchanged
    and is what the Filing tab asks. What changes is that drafting no longer stops.
    """
    return _blockers(report, drawings=False, text=True)


def drawing_blockers(report: Mapping[str, Any]) -> list[str]:
    """What is wrong with the sheets, reported rather than raised."""
...
class TurnRunner:
    ...
    def _run_agent(self, *, turn_id: int, lease: str, workspace: Path, prompt: str,
                   ...
                   tools: str = "") -> draft_agent.AgentRun:
    ...
    def _reconcile_drawings(self, *, turn_id: int, lease: str, project_id: int, user_id: int,
        ...
        """Bring the sheets into line with the text if that fits in the budget, and say what did not.

        NEVER raises for a drawing reason. Three things still travel through it, and each is about
        the TEXT or about the user rather than about a sheet: the user pressing Stop, a
        source-fidelity finding (the reviewer reading the draft against the disclosure), and the
        reviewer being unavailable at all, which is fail-closed on purpose and retries the turn.
        """
        #  THE PASS THAT DOES NOT NEED TO RUN. Every stored audit carries the specification hash it
        #  was taken against. When each sheet's active image was already inspected, and passed,
        #  against exactly the brief in hand, there is nothing about the DRAWINGS left to decide:
        #  re-running the pipeline only re-applies a gate whose VERSION moved. That is the whole
        #  53 minutes measured on project 8, on sheets whose hashes matched all along.
        ...
        try:
            generated = self._ensure_figures(
                ...
                sections=sections, numerals=numerals, figures=figures,
                disclosure=disclosure, workspace=workspace, deadline=deadline)
        ...
        def check_cancel() -> None:
            if deadline and time.time() > deadline:
                raise DrawingBudgetSpent(
                    "The drawing pass reached its time budget for this turn.")
    ...
    def _model_for(self, project: Mapping[str, Any]) -> str:
        ...
        return draft_agent.normalize_model(project.get("draft_model"))
    ...
    def run(self, turn: Mapping[str, Any]) -> dict[str, Any]:
        ...
        #  ONE budget for the whole turn. Per-round was still six passes on a six-round repair, so
        #  a turn could spend an hour and a half on sheets while the text sat finished.
        drawing_deadline = time.time() + DRAWING_BUDGET_SECONDS
        for review_index in range(MAX_FINALIZATION_ROUNDS):
            ...
            if answered_without_candidate and review_index == 0:
                ...
                try:
                    ...
                    #  DRAWINGS ARE ATTEMPTED, NOT REQUIRED. Whatever comes back, the turn goes on
                    #  to the review and can publish the text; what the sheets are missing is
                    #  carried into the report as a drawing defect, which is where the Filing gate
                    #  reads it from. The budget is what stops a bumped audit version turning every
                    #  wording change into an hour of re-inspection.
                    self.repository.heartbeat(
                        turn_id, lease, stage="drawing and inspecting figures")
                    drawing_faults.extend(self._reconcile_drawings(
                        ...
                        workspace=workspace, deadline=drawing_deadline))
            ...
            #  TEXT blockers only. A drawing defect is reported, not repaired in this loop, and
            #  never stops the version being published.
