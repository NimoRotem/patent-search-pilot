...
def build_repair_prompt(workspace: Path, report: Mapping[str, Any]) -> tuple[str, bool]:
    ...
    return REPAIR_PROMPT % {"materials": materials, "report": body}, True
...
class TurnRunner:
    ...
    def _reconcile_drawings(self, *, turn_id: int, lease: str, project_id: int, user_id: int,
        ...
        try:
            ...
            raise
        except DrawingBudgetSpent as exc:
            return [f"{exc} The sheets that were reached are up to date; the rest are not."]
    ...
    def _review_sources(self, *, turn_id: int, lease: str, sections: Mapping[str, str],
        ...
        if report is None:
            ...
            outcome = self.qa.review_sources(workspace, transcript=transcript, model=model)
        ...
        for review_index in range(rounds_allowed):
            ...
            if patch_miss is not None:
                ...
                try:
                    ...
                    #  A DRAFTING TURN DOES NOT DRAW. Owner's instruction, 2026-08-26, after
                    #  watching this stage run without end: "Completely separate the figures and
                    #  sketch process from the drafting."
                    #
                    #  It is not only slow, it cannot converge. The audits are versioned, so one
                    #  bumped constant in draft_figures invalidates every stored verdict and every
                    #  sheet is re-inspected from scratch on every turn for ever - measured at 190
                    #  inspection calls and 53 minutes of model time for a one-sentence change, on
                    #  sheets whose specification hashes matched all along. One turn sat in this
                    #  stage for NINE HOURS.
                    #
                    #  Drawings are still filing artifacts and are still refused at the filing
                    #  gate: `filing_blockers` is unchanged and the Filing tab asks it. They are
                    #  produced by their own explicit pass instead, which is where the waiting now
                    #  belongs. Whoever changes this back: the text must publish on the text's
                    #  merits, or every wording change waits on an image pipeline again.
                    #  The source-fidelity gate is a TEXT gate and runs on its own now. It used
                    #  to sit inside the drawing pass, so taking drawings out of a drafting turn
                    #  silently took this with them, and it is the check that catches the agent
                    #  inventing a structure or a definition to make a claim work.
                    self.repository.heartbeat(
                    ...
                    report = self.evaluate(
                        ...
                        numerals=snapshot["numerals"], figures=snapshot["figures"],
                        review_index=review_index, turn_id=turn_id,
            ...
            #  TEXT blockers only. What is wrong with the sheets is reported, and still refuses a
            #  filing package, but it never stops a version of the TEXT being published.
            blockers = text_blockers(report)
            if not blockers:
                break
