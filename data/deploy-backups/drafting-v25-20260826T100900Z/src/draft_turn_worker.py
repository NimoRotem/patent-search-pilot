...
def run(stop_event: threading.Event | None = None) -> None:
    ...
    #  The extra slots are daemon threads and the calling thread is the first one, so SIGTERM
    #  still reaches this process directly and a drain still ends when the main loop returns.
