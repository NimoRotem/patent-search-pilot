...
def _with_vertex_fallback(common: Mapping[str, Any], previous: AgentRun) -> AgentRun:
    ...
    if not state.degraded and not state.allowed:
        ...
        return _merge_attempts(
            ...
            "Every Claude subscription was out of quota and the metered fallback is at its "
    ...
    return _merge_attempts(
        previous, vertex,
        "Every Claude subscription on this host was out of quota, so the run continued through "
...
def run(*, workspace: Path, prompt: str, system_prompt: str, schema: Mapping[str, Any],
    ...
    for index, token in enumerate(tokens):
        ...
        if merged is None:
            ...
            merged = _merge_attempts(
                merged, current,
                f"Subscription {index} was out of quota, so the run continued on subscription "
        ...
        if merged.ok or merged.cancelled or (cancel is not None and cancel.is_set()):
            return merged
        if not _subscription_limit_error(merged.error):
    ...
    merged.steps = merged.steps + [{
        "kind": "system",
        "text": (f"Every configured subscription ({len(tokens)}) is out of quota. Metered API "
                 f"billing is deliberately not a fallback, so nothing was billed. The draft is "
                 f"saved and the turn can be run again when a plan window reopens.")}]
    return _with_vertex_fallback(common, merged)
