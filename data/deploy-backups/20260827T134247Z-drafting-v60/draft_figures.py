...
#  No fallback import: this pipeline must not call a metered API unmetered.
...
def _anthropic_endpoint_message(payload: dict, *, api_key: str) -> dict:
    ...
    #  The daily ceiling. Raising here rather than returning empty keeps the fail-closed contract
    #  for a REQUIRED crosscheck: a verification gate that silently skips is worse than one that
    #  stops the pass, and the optional stages already treat an error as "not reviewed".
    _SPEND.check()
    ...
    for attempt in range(3):
        ...
        try:
            ...
            _SPEND.record(model=str(payload.get("model") or ""),
                          usage=value.get("usage") or {}, route="api",
                          detail="figure-crosscheck")
            return value
