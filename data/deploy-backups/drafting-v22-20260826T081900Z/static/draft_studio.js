  ...
  function renderFigures() {
    ...
    $('figuresBody').innerHTML = `
      ...
      <div class="drawreconcile">
        <div><b>Bring every sheet into line with the text</b>
          <div class="small muted">Drawings no longer hold up a wording change: the text publishes
            on its own merits and what the sheets are missing is reported here and under Filing.
            This runs the image pipeline against the published draft, with no drafting agent.</div></div>
        <button type="button" class="btn sm" id="figReconcile">Reconcile drawings</button>
  ...
  function renderBusy() {
    ...
    if (turn) {
      ...
      $('chatStage').textContent =
        'reconciling the drawings with the published text - the draft is not waiting on this';
    } else if (reviewing) {
